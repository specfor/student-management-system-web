...
let props = defineProps(["totalCount", "pageSize", "currentPage"]);

let totalCount = toRef(props, "totalCount");
let pageSize = toRef(props, "pageSize");
let currentPage = props.currentPage ? toRef(props, "currentPage") : ref(1);
...
function loadPage(pageNum: number) {
    ...
    if (!props.currentPage) currentPage.value = pageNum;
}
        ...
        <ul class="flex justify-center items-center -space-x-px h-8 text-sm relative">
            ...
            <div class="absolute right-0 text-md">
                <div class="border py-1 px-3 rounded-md text-slate-600">
                    <p>
                        Records Count: <span class="text-blue-500 font-bold">{{ props.totalCount }}</span>
                    </p>
                </div>
            </div>
        </ul>

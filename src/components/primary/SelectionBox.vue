...
<template>
    <div class="relative" v-click-outside="() => { if (isOpen) isOpen = false }"
        @keydown.esc="(event) => { if (isOpen) event.preventDefault(); isOpen = false }"
        ...
        <div tabindex="0" @click="() => { isOpen = !isOpen }"
            class="border min-h-9 border-gray-400 bg-white hover:bg-gray-100 rounded-md w-auto py-1 flex justify-between items-center">
        ...
        <div class="absolute w-full bg-white shadow-lg z-30" v-show="isOpen">
            <div class="relative mt-2 mb-3 mx-1">
                ...
                <input type="text" :value="search" @input="(event) => { search = event.target.value; searchItem() }"
            ...
            <div class="max-h-96 h-fit border overflow-y-auto">

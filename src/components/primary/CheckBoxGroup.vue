...
<template>
    <div v-for="(option, index3) in props.options" :key="index3" class="flex mt-1">
        <input class="w-5 h-5 mt-0.5" :disabled="props.disabled" type="checkbox" :checked="option['checked']" @input="event => {
            values[option['name']] = event.target.checked; emit('change', values)
        }" />

...
async function justRemovePaymentFromBill() {
    ...
    if (resp.status == 'success')
        ...
        alertStore.insertAlert("Failed to Remove Payment", resp.message, 'error')
...
async function refundAndRemovePayment() {
    ...
    if (refundReason.value.length == 0) {
        ...
        if (resp.status == 'success')
            ...
            alertStore.insertAlert("Failed to Refund Payment", resp.message, 'error')
    }
...
<template>
    ...
    <div class="flex mt-10 justify-end gap-3 text-white bg-slate-200 py-3 px-5">
        <button
            ...
            :disabled="buttonsDisabled" @click="refundAndRemovePayment">Refund & Remove</button>
        <button
            class="border py-1 px-4 rounded-md bg-violet-600 hover:bg-violet-800 active:bg-violet-900 disabled:bg-violet-300"
            :disabled="buttonsDisabled" @click="justRemovePaymentFromBill">Remove</button>
        <button
            class="border py-1 px-4 rounded-md bg-violet-600 hover:bg-violet-800 active:bg-violet-900 disabled:bg-violet-300"
            :disabled="buttonsDisabled" @click="() => { close() }">Cancel</button>
    </div>
</template>
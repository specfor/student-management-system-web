...
if (props.expand) expanded.value = true;

const emit = defineEmits<{
  actionComplete: [action: typeof props.status];
  removePayment: [paymentId: number, billId: number];
}>();
...
async function printBill(status: typeof props.status) {
  ...
  if (error.occured) alertStore.insertAlert(error.header, error.body, "error");
  ...
  <div class="py-2 px-3 text-md bg-slate-100 rounded-md">
    ...
    <div class="grid grid-cols-3 gap-x-5 mt-2 text-sm" v-if="!hideButtons">
      ...
      <div class="flex relative text-white">
        <button
          class="flex items-center justify-center gap-2 bg-green-600 px-2 py-1 rounded-l-sm hover:cursor-pointer active:bg-green-800 disabled:bg-green-300 disabled:hover:cursor-not-allowed w-full"
          @click="
            () => {
              printBill('printed');
            }
        ...
        <button
          class="w-fit flex items-center justify-center bg-green-600 active:bg-green-800 disabled:bg-green-300 disabled:hover:cursor-not-allowed px-2 cursor-pointer border-l"
          :disabled="disableBtns"
        >
      ...
      <button
        class="flex items-center justify-center gap-2 px-2 py-1 rounded-sm text-white hover:cursor-pointer bg-red-600 active:bg-red-800 disabled:bg-red-300 disabled:hover:cursor-not-allowed"
        @click="
          () => {
            printBill('cancelled');
          }
    ...
    <div v-show="expanded" v-if="paymentData">
      ...
      <div class="mx-2">
        ...
        <div
          class="grid grid-cols-[3fr_1fr_1fr_10px] items-center"
          v-for="(payment, index) in props.paymentData"
          :key="index"
        >
          ...
          <MinusCircleIcon
            @click="$emit('removePayment', payment.paymentId, props.billId)"

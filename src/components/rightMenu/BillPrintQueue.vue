...
const filters: Ref<{
  printStatus: "pending" | "cancelled" | "printed";
...
const selectedPaymentToRemove: Ref<BillPaymentManagerProps["payment"]> = ref({
  amount: "",
  course: "",
  student: "",
  id: 0,
...
async function loadBills() {
  ...
  let resp = await getBills(queryStartIndex, paginatePageSize, {
    ...
    filters: {
      status: filters.value.printStatus,
      bill_id: filters.value.billId,
      updated_month: filters.value.issuedMonth,
    },
  });
  if (resp.status == "success") {
    bills.value = [];
    await Promise.all(
      (resp.data.bills as StudentPaymentBill[]).map(async (bill) => {
        ...
        if (bill.admission_payment) {
          b.paymentData.push({
            paymentId: -1,
            courseName: bill.admission_payment.printName,
            paymentFor: bill.admission_payment.paymentFor,
            amount: bill.admission_payment.amount,
          });
        }
        bills.value.push(b);
      })
...
loadBills();
globalDataStore.insertHook("refresh-receipt-list", () => {
  queryStartIndex = 0;
  loadBills();
});
...
function flipShowAllBills() {
  if (!showingAllBills.value) shownBills.value = bills.value;
  else shownBills.value = bills.value.slice(0, 1);
...
<template>
  <div class="w-full h-full max-h-[95%] p-4 mb-4">
    ...
    <div class="mb-3">
      ...
      <div class="grid grid-cols-2 gap-x-5 items-center ml-5">
        ...
        <div class="flex items-center justify-evenly">
          ...
          <input
            type="month"
            class="border border-slate-600 py-1 px-2 ml-4 rounded-lg w-36"
      ...
      <div class="flex justify-end gap-x-4 mt-2">
        <button
          @click="resetFilters"
          class="font-semibold bg-stone-600 hover:bg-stone-800 w-fit px-4 justify-self-end py-1 text-white rounded-md"
        >
          Reset Filters
        </button>
        <button
    ...
    <div v-show="shownBills.length !== 0 && !loadingBills" class="overflow-y-auto h-[80%]">
      <template v-for="(bill, index) in shownBills" :key="bill.id">
        <BillCard
          :bill-id="bill.id"
          :bill-value="bill.billValue"
          :payment-data="bill.paymentData"
          :student-id="bill.studentId"
          :student-custom-id="bill.studentCustomId"
          :stduent-name="bill.studentName"
          :status="bill.status"
          class="mt-3"
          @action-complete="
            (status) => {
              removeBill(bill.id, status);
            }
          "
          :expand="index == 0"
        ...
        <div class="flex justify-center" v-show="index == 0">
          <button @click="flipShowAllBills" class="my-2 border px-3 bg-gray-400 text-black">
            {{ showingAllBills ? "Hide" : "Show All" }}
          </button>
        </div>
      </template>
      <PaginateComponent
        :total-count="paginateTotal"
        :page-size="paginatePageSize"
        @load-page-emit="
          (startIndex) => {
  ...
  <PopUpPlaceholder :show="showBillPaymentManager">
    <BillPaymentManager
      :payment="selectedPaymentToRemove"
      @close="showBillPaymentManager = false"
      @close-and-refresh-bills="
        () => {
          showBillPaymentManager = false;
          loadBills();
        }
      "
    />

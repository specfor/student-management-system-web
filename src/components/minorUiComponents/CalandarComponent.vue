...
async function updateClassConduct() {
    ...
    if (resp.status === "success") {
        ...
        const index = showCards.value.findIndex(
            (card) =>
                card.title === selectedCard.value?.title && card.date === selectedCard.value?.date
        );
        if (index !== -1) {
        ...
        alertStore.insertAlert(
            "Error",
            "Failed to update class schedule: " + resp.message,
            "error"
        );
    }
}
    ...
    <div class="p-4">
        <div class="flex items-center mb-4 gap-2">
            ...
            <span class="font-bold text-lg">
                ...
                <template v-else>
                    {{
                        days.find((d) => d !== null)?.toLocaleString("default", { month: "long" })
                    }}
                    {{ days.find((d) => d !== null)?.getFullYear() }}
                </template>
            </span>
            ...
            <button
                ...
                :class="[
                    'px-2 py-1 rounded',
                    mode === 'week' ? 'bg-blue-500 text-white' : 'bg-gray-200',
                ]"
            >
                Week
            </button>
        ...
        <div
            :class="mode === 'week' ? 'grid grid-cols-7 gap-2' : 'grid grid-cols-7 gap-2'"
            class="max-h-[800px] overflow-y-auto"
        >
            <div
                v-for="(day, index) in days"
                :key="day ? day.toISOString() : `empty-${index}`"
                class="border rounded p-2 min-h-[120px] flex flex-col"
            >
                ...
                <template v-if="day">
                    <div
                        v-for="card in getCardsForDate(formatDate(day))"
                        :key="card.title + card.timeRange"
                ...
                <div class="p-4">
                    ...
                    <div class="mb-4 mt-8 grid grid-cols-3">
                        ...
                        <select
                            ...
                            class="border border-gray-400 col-span-2 rounded p-1 w-full"
                        >
                    ...
                    <div class="mb-4 grid grid-cols-3">
                        ...
                        <textarea
                            v-model="notes"
                            class="border border-gray-400 rounded col-span-2 p-1 w-full"
                            rows="2"
                    ...
                    <div class="mt-4">
                        <button
                            ...
                            class="px-4 py-2 bg-yellow-400 hover:bg-yellow-600 rounded w-full"
                        >
                            Reschedule Class
                        </button>
                        <div v-if="showReschedule" class="mt-4 border-t pt-4">
                            ...
                            <div class="flex justify-end gap-2">
                                <button
                                    ...
                                    class="px-4 py-2 bg-gray-400 hover:bg-gray-600 rounded"
                                >
                                    Cancel
                                </button>
                                <button
                                    @click="handleReschedule"
                                    :disabled="handlingReschedule"
                                    class="px-4 py-2 bg-yellow-400 hover:bg-yellow-600 rounded disabled:bg-yellow-200"
                                >
                                    Confirm Reschedule
                                </button>
                            </div>

    ...
    <TransitionGroup name="notifications">
      <div v-for="notification in alertStore.alerts" :key="notification['id']">
        <div v-if="notification['type'] === 'success'"
          ...
          :class="{ 'cursor-pointer': notification.onClick }"
          @click="if (notification.onClick) { notification.onClick(); removeNotification(notification.id) }">
          ...
          <XMarkIcon
            class="stroke-green-800 stroke-2 w-[25px] h-[25px] hover:border-2 hover:border-green-600 mt-1 justify-self-end cursor-pointer"
        ...
        <div v-else-if="notification['type'] === 'error'"
          ...
          <XMarkIcon
            class="stroke-rose-800 stroke-2 w-[25px] h-[25px] hover:border-2 hover:border-rose-600 mt-1 justify-self-end cursor-pointer"
            @click.stop="removeNotification(notification['id'])" />
        </div>
        <div v-else-if="notification['type'] === 'info'"
          ...
          <XMarkIcon
            class="stroke-blue-800 stroke-2 w-[25px] h-[25px] hover:border-2 hover:border-blue-600 mt-1 justify-self-end cursor-pointer"
            @click.stop="removeNotification(notification['id'])" />
        </div>
      </div>
    </TransitionGroup>
